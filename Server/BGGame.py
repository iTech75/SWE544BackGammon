...
class BGGame(threading.Thread):
    ...
    def __init__(self, bgserver, black, white):
        ...
        self.__gameStatus = [GamePoint(0, "", 0)] * 26
    ...
    def __create_game_status_string(self):
        ...
        for i in range(1, 25):
            if self.__gameStatus[i] != "":
                result += "|%d%s" % (i, self.__gameStatus[i])
        return result
    ...
    def __parse_backgammon_notation(self, move, roll):
        ...
        for item in data:
            ...
            if source_target.__len__ == 2:
                ...
                if self.__gameStatus[int(source_target[0])].count > 0:
                    if roll == self.blackPlayer:
                        self.__gameStatus[int(source_target[0])].color = "b"
                    else:
                        self.__gameStatus[int(source_target[0])].color = "w"
